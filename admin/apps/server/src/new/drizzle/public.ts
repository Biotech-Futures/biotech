import { pgTable, bigint, varchar, timestamp, index, unique, foreignKey, integer, check, boolean, uniqueIndex, smallint, time, text, jsonb, date, numeric, interval, pgSchema, serial } from "drizzle-orm/pg-core"
...
export const countries = pgTable("countries", {
	...
	index("countries_country_name_3eac3d42_like").using("btree", table.countryName.asc().nullsLast().op("varchar_pattern_ops")),
...
export const tracks = pgTable("tracks", {
	...
	index("tracks_state_i_8ee9cb_idx").using("btree", table.stateId.asc().nullsLast().op("int8_ops")),
	index("tracks_state_id_46f6a1ec").using("btree", table.stateId.asc().nullsLast().op("int8_ops")),
	index("tracks_track_name_cf9addea_like").using("btree", table.trackName.asc().nullsLast().op("varchar_pattern_ops")),
	foreignKey({
...
export const studentProfile = pgTable("student_profile", {
	...
	index("student_pro_supervi_0372b3_idx").using("btree", table.supervisorId.asc().nullsLast().op("int8_ops")),
	index("student_profile_supervisor_id_4bc6991c").using("btree", table.supervisorId.asc().nullsLast().op("int8_ops")),
	foreignKey({
	...
	check("student_year_lvl_valid", sql`(year_lvl)::text = ANY ((ARRAY['9'::character varying, '10'::character varying, '11'::character varying, '12'::character varying])::text[])`),
]);
...
export const users = pgTable("users", {
	...
	index("users_account_33da0c_idx").using("btree", table.accountStatus.asc().nullsLast().op("text_ops")),
	index("users_email_0ea73cca_like").using("btree", table.email.asc().nullsLast().op("varchar_pattern_ops")),
	index("users_track_i_1a6677_idx").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("users_track_id_e7963e8d").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const adminScope = pgTable("admin_scope", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "admin_scope_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("admin_scope_track_i_594c81_idx").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("admin_scope_track_id_633443ef").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("admin_scope_user_id_4ad1f0_idx").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	index("admin_scope_user_id_c9b7737b").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	uniqueIndex("unique_admin_scope_per_track").using("btree", table.userId.asc().nullsLast().op("int8_ops"), table.trackId.asc().nullsLast().op("int8_ops")).where(sql`(NOT is_global)`),
	uniqueIndex("unique_global_admin_scope").using("btree", table.userId.asc().nullsLast().op("int8_ops")).where(sql`is_global`),
	foreignKey({
			columns: [table.trackId],
			foreignColumns: [tracks.id],
	...
	check("admin_scope_global_or_track", sql`(is_global AND (track_id IS NULL)) OR ((NOT is_global) AND (track_id IS NOT NULL))`),
]);

export const mentorAvailability = pgTable("mentor_availability", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "mentor_availability_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("mentor_avai_mentor__83178e_idx").using("btree", table.mentorUserId.asc().nullsLast().op("int8_ops")),
	index("mentor_avai_weekday_25332b_idx").using("btree", table.weekday.asc().nullsLast().op("int2_ops")),
	index("mentor_availability_mentor_user_id_f756a8ac").using("btree", table.mentorUserId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.mentorUserId],
			foreignColumns: [users.id],
			name: "mentor_availability_mentor_user_id_f756a8ac_fk_users_id"
		}),
	unique("unique_mentor_availability_slot").on(table.weekday, table.startTime, table.endTime, table.mentorUserId),
	check("mentor_availability_end_after_start", sql`end_time > start_time`),
	check("mentor_availability_weekday_check", sql`weekday >= 0`),
	check("mentor_availability_weekday_valid", sql`(weekday >= 0) AND (weekday <= 6)`),
]);
...
export const userInterest = pgTable("user_interest", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "user_interest_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	interestId: bigint("interest_id", { mode: "number" }).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	userId: bigint("user_id", { mode: "number" }).notNull(),
}, (table) => [
	index("user_intere_interes_4522f5_idx").using("btree", table.interestId.asc().nullsLast().op("int8_ops")),
	index("user_intere_user_id_169ea4_idx").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	index("user_interest_interest_id_0953fed5").using("btree", table.interestId.asc().nullsLast().op("int8_ops")),
	index("user_interest_user_id_5b53cab9").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const studentSupervisor = pgTable("student_supervisor", {
	...
	index("student_sup_student_0afdc5_idx").using("btree", table.studentUserId.asc().nullsLast().op("int8_ops")),
	index("student_sup_supervi_51462f_idx").using("btree", table.supervisorUserId.asc().nullsLast().op("int8_ops")),
	index("student_supervisor_student_user_id_fb056317").using("btree", table.studentUserId.asc().nullsLast().op("int8_ops")),
	index("student_supervisor_supervisor_user_id_8a5fb4d1").using("btree", table.supervisorUserId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const resourceTypes = pgTable("resource_types", {
	...
	index("resource_types_type_name_368352d8_like").using("btree", table.typeName.asc().nullsLast().op("varchar_pattern_ops")),
...
export const roles = pgTable("roles", {
	...
	index("roles_role_name_2c8c2fcd_like").using("btree", table.roleName.asc().nullsLast().op("varchar_pattern_ops")),
...
export const announcements = pgTable("announcements", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "announcements_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("announcemen_archive_36591b_idx").using("btree", table.archivedAt.asc().nullsLast().op("timestamptz_ops")),
	index("announcemen_author__403994_idx").using("btree", table.authorUserId.asc().nullsLast().op("int8_ops")),
	index("announcemen_publish_6b0e06_idx").using("btree", table.publishedAt.asc().nullsLast().op("timestamptz_ops")),
	index("announcemen_track_i_c21806_idx").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("announcemen_visibil_a80d79_idx").using("btree", table.visibilityScope.asc().nullsLast().op("text_ops")),
	index("announcements_author_user_id_54f6a903").using("btree", table.authorUserId.asc().nullsLast().op("int8_ops")),
	index("announcements_track_id_ba24ce92").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	foreignKey({
	...
	check("announcement_archived_after_published", sql`(archived_at >= published_at) OR (archived_at IS NULL)`),
]);

export const announcementAudience = pgTable("announcement_audience", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "announcement_audience_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	index("announcemen_announc_d9cb5b_idx").using("btree", table.announcementId.asc().nullsLast().op("int8_ops")),
	index("announcemen_role_id_1076c4_idx").using("btree", table.roleId.asc().nullsLast().op("int8_ops")),
	index("announcemen_track_i_391dbc_idx").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("announcement_audience_announcement_id_25bd1712").using("btree", table.announcementId.asc().nullsLast().op("int8_ops")),
	index("announcement_audience_role_id_571d0f03").using("btree", table.roleId.asc().nullsLast().op("int8_ops")),
	index("announcement_audience_track_id_5b49af8c").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	uniqueIndex("unique_announcement_role_audience").using("btree", table.announcementId.asc().nullsLast().op("int8_ops"), table.roleId.asc().nullsLast().op("int8_ops")).where(sql`((track_id IS NULL) AND (role_id IS NOT NULL))`),
	uniqueIndex("unique_announcement_role_track_audience").using("btree", table.announcementId.asc().nullsLast().op("int8_ops"), table.roleId.asc().nullsLast().op("int8_ops"), table.trackId.asc().nullsLast().op("int8_ops")).where(sql`((role_id IS NOT NULL) AND (track_id IS NOT NULL))`),
	uniqueIndex("unique_announcement_track_audience").using("btree", table.announcementId.asc().nullsLast().op("int8_ops"), table.trackId.asc().nullsLast().op("int8_ops")).where(sql`((role_id IS NULL) AND (track_id IS NOT NULL))`),
	foreignKey({
	...
	check("announcement_audience_requires_role_or_track", sql`(role_id IS NOT NULL) OR (track_id IS NOT NULL)`),
]);

export const auditLog = pgTable("audit_log", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "audit_log_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("audit_log_action_b32d4d_idx").using("btree", table.action.asc().nullsLast().op("text_ops")),
	index("audit_log_actor_u_ae2850_idx").using("btree", table.actorUserId.asc().nullsLast().op("int8_ops")),
	index("audit_log_actor_user_id_147aa761").using("btree", table.actorUserId.asc().nullsLast().op("int8_ops")),
	index("audit_log_created_5a7975_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamptz_ops")),
	index("audit_log_entity__c2633a_idx").using("btree", table.entityType.asc().nullsLast().op("text_ops"), table.entityId.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const mentorCertificate = pgTable("mentor_certificate", {
	...
	index("mentor_cert_mentor__f7358b_idx").using("btree", table.mentorProfileId.asc().nullsLast().op("int8_ops"), table.certificateTypeId.asc().nullsLast().op("int8_ops")),
	index("mentor_certificate_certificate_type_id_090af26f").using("btree", table.certificateTypeId.asc().nullsLast().op("int8_ops")),
	index("mentor_certificate_mentor_profile_id_61bbc118").using("btree", table.mentorProfileId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const messageResources = pgTable("message_resources", {
	...
	index("message_res_message_8048ce_idx").using("btree", table.messageId.asc().nullsLast().op("int8_ops")),
	index("message_res_resourc_d3776f_idx").using("btree", table.resourceId.asc().nullsLast().op("int8_ops")),
	index("message_resources_message_id_e0b74b54").using("btree", table.messageId.asc().nullsLast().op("int8_ops")),
	index("message_resources_resource_id_7bf8fbc9").using("btree", table.resourceId.asc().nullsLast().op("int8_ops")),
	foreignKey({
	...
	unique("message_resources_message_id_resource_id_9a04fab7_uniq").on(table.resourceId, table.messageId),
]);
...
export const messageStatus = pgTable("message_status", {
	...
	index("message_sta_message_5f9800_idx").using("btree", table.messageId.asc().nullsLast().op("int8_ops")),
	index("message_sta_user_id_2503ad_idx").using("btree", table.userId.asc().nullsLast().op("int8_ops"), table.status.asc().nullsLast().op("text_ops")),
	index("message_status_message_id_7d8e74e7").using("btree", table.messageId.asc().nullsLast().op("int8_ops")),
	index("message_status_user_id_0673798e").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.messageId],
			foreignColumns: [messages.id],
...
export const messages = pgTable("messages", {
	...
	index("messages_deleted_555c2c_idx").using("btree", table.deletedAt.asc().nullsLast().op("timestamptz_ops")),
	index("messages_group_i_18591d_idx").using("btree", table.groupId.asc().nullsLast().op("int8_ops"), table.sentAt.asc().nullsLast().op("int8_ops")),
	index("messages_group_id_0fd96228").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	index("messages_sender__199d51_idx").using("btree", table.senderUserId.asc().nullsLast().op("int8_ops")),
	index("messages_sender_user_id_b1b10099").using("btree", table.senderUserId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.groupId],
...
export const events = pgTable("events", {
	...
	index("events_track_i_d7b306_idx").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("events_track_id_a0bf15c2").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const eventTargetRole = pgTable("event_target_role", {
	...
	index("event_targe_event_i_bcd600_idx").using("btree", table.eventId.asc().nullsLast().op("int8_ops")),
	index("event_targe_role_id_6f7b73_idx").using("btree", table.roleId.asc().nullsLast().op("int8_ops")),
...
export const eventTargetTrack = pgTable("event_target_track", {
	...
	index("event_targe_event_i_2f7445_idx").using("btree", table.eventId.asc().nullsLast().op("int8_ops")),
	index("event_targe_track_i_96c57e_idx").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
...
export const eventRsvp = pgTable("event_rsvp", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "event_rsvp_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("event_rsvp_event_i_393d63_idx").using("btree", table.eventId.asc().nullsLast().op("int8_ops")),
	index("event_rsvp_event_id_8ccc26b8").using("btree", table.eventId.asc().nullsLast().op("int8_ops")),
	index("event_rsvp_rsvp_st_80e8a8_idx").using("btree", table.rsvpStatus.asc().nullsLast().op("text_ops")),
	index("event_rsvp_user_id_59b81e_idx").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	index("event_rsvp_user_id_c3df2ffd").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.eventId],
			foreignColumns: [events.id],
	...
	check("event_rsvp_response_state_valid", sql`(responded_at IS NULL) OR ((rsvp_status)::text = ANY ((ARRAY['going'::character varying, 'maybe'::character varying, 'declined'::character varying])::text[]))`),
]);

export const groupMembership = pgTable("group_membership", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "group_membership_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("group_membe_group_i_e52645_idx").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	index("group_membe_left_at_e9e6a8_idx").using("btree", table.leftAt.asc().nullsLast().op("timestamptz_ops")),
	index("group_membe_user_id_9eabd2_idx").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	index("group_membership_group_id_45c6964f").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	index("group_membership_user_id_35a27eaa").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	uniqueIndex("unique_active_group_membership").using("btree", table.groupId.asc().nullsLast().op("int8_ops"), table.userId.asc().nullsLast().op("int8_ops")).where(sql`(left_at IS NULL)`),
	foreignKey({
			columns: [table.groupId],
			foreignColumns: [groups.id],
	...
	check("group_membership_left_after_joined", sql`(left_at >= joined_at) OR (left_at IS NULL)`),
]);
...
export const groups = pgTable("groups", {
	...
	index("groups_created_d34ebd_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamptz_ops")),
	index("groups_deleted_8ee2fc_idx").using("btree", table.deletedAt.asc().nullsLast().op("timestamptz_ops")),
	...
	uniqueIndex("unique_active_group_name_per_track").using("btree", table.trackId.asc().nullsLast().op("text_ops"), table.groupName.asc().nullsLast().op("int8_ops")).where(sql`(deleted_at IS NULL)`),
	foreignKey({
			columns: [table.trackId],
			foreignColumns: [tracks.id],
			name: "groups_track_id_f29fde9a_fk_tracks_id"
		}),
	check("group_deleted_after_created", sql`(deleted_at >= created_at) OR (deleted_at IS NULL)`),
	check("group_name_not_empty", sql`NOT ((group_name)::text ~ '^\s*$'::text)`),
]);

export const matchingMentor = pgTable("matching_mentor", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matching_mentor_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("matching_mentor_email_a48af002_like").using("btree", table.email.asc().nullsLast().op("varchar_pattern_ops")),
	unique("matching_mentor_email_key").on(table.email),
	check("matching_mentor_max_groups_check", sql`max_groups >= 0`),
]);

export const matchingMentorInterests = pgTable("matching_mentor_interests", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matching_mentor_interests_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	mentorId: bigint("mentor_id", { mode: "number" }).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	interestId: bigint("interest_id", { mode: "number" }).notNull(),
}, (table) => [
	index("matching_mentor_interests_interest_id_9292dd16").using("btree", table.interestId.asc().nullsLast().op("int8_ops")),
	index("matching_mentor_interests_mentor_id_d5e11250").using("btree", table.mentorId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.interestId],
	...
	unique("matching_mentor_interests_mentor_id_interest_id_ad1fe674_uniq").on(table.mentorId, table.interestId),
]);

export const matchingInterest = pgTable("matching_interest", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matching_interest_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	name: varchar({ length: 80 }).notNull(),
}, (table) => [
	index("matching_interest_name_860b2179_like").using("btree", table.name.asc().nullsLast().op("varchar_pattern_ops")),
	unique("matching_interest_name_key").on(table.name),
]);

export const matchingStudent = pgTable("matching_student", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matching_student_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	firstName: varchar("first_name", { length: 80 }).notNull(),
	lastName: varchar("last_name", { length: 80 }).notNull(),
	email: varchar({ length: 254 }).notNull(),
	...
	index("matching_student_email_aa9b61cf_like").using("btree", table.email.asc().nullsLast().op("varchar_pattern_ops")),
	unique("matching_student_email_key").on(table.email),
	check("matching_student_year_level_check", sql`year_level >= 0`),
]);

export const matchingStudentInterests = pgTable("matching_student_interests", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matching_student_interests_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	studentId: bigint("student_id", { mode: "number" }).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	interestId: bigint("interest_id", { mode: "number" }).notNull(),
}, (table) => [
	index("matching_student_interests_interest_id_b50a0b8a").using("btree", table.interestId.asc().nullsLast().op("int8_ops")),
	index("matching_student_interests_student_id_0d287558").using("btree", table.studentId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.interestId],
			foreignColumns: [matchingInterest.id],
	...
	unique("matching_student_interests_student_id_interest_id_df98b935_uniq").on(table.studentId, table.interestId),
]);

export const matchingStudentgroup = pgTable("matching_studentgroup", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matching_studentgroup_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("matching_studentgroup_mentor_id_318e9d58").using("btree", table.mentorId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.mentorId],
			foreignColumns: [matchingMentor.id],
...
export const matchingStudentgroupInterests = pgTable("matching_studentgroup_interests", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matching_studentgroup_interests_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	studentgroupId: bigint("studentgroup_id", { mode: "number" }).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	interestId: bigint("interest_id", { mode: "number" }).notNull(),
}, (table) => [
	index("matching_studentgroup_interests_interest_id_39fc584d").using("btree", table.interestId.asc().nullsLast().op("int8_ops")),
	index("matching_studentgroup_interests_studentgroup_id_de9a8747").using("btree", table.studentgroupId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.interestId],
			foreignColumns: [matchingInterest.id],
	...
	unique("matching_studentgroup_in_studentgroup_id_interest_79199744_uniq").on(table.studentgroupId, table.interestId),
]);

export const matchingStudentgroupMembers = pgTable("matching_studentgroup_members", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matching_studentgroup_members_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	studentgroupId: bigint("studentgroup_id", { mode: "number" }).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	studentId: bigint("student_id", { mode: "number" }).notNull(),
}, (table) => [
	index("matching_studentgroup_members_student_id_f22f0640").using("btree", table.studentId.asc().nullsLast().op("int8_ops")),
	index("matching_studentgroup_members_studentgroup_id_010376c8").using("btree", table.studentgroupId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.studentId],
			foreignColumns: [matchingStudent.id],
	...
	unique("matching_studentgroup_me_studentgroup_id_student__3bdd378f_uniq").on(table.studentgroupId, table.studentId),
]);

export const matchingMentoravailability = pgTable("matching_mentoravailability", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matching_mentoravailability_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	weekday: smallint().notNull(),
	start: time().notNull(),
	end: time().notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	index("matching_mentoravailability_mentor_id_468a5bf9").using("btree", table.mentorId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.mentorId],
			foreignColumns: [matchingMentor.id],
			name: "matching_mentoravail_mentor_id_468a5bf9_fk_matching_"
		}),
	unique("matching_mentoravailabil_mentor_id_weekday_start__9e83be24_uniq").on(table.weekday, table.start, table.end, table.mentorId),
	check("matching_mentoravailability_weekday_check", sql`weekday >= 0`),
]);

export const matchRun = pgTable("match_run", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "match_run_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("match_run_created_ea68ea_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamptz_ops")),
	index("match_run_initiat_12e56d_idx").using("btree", table.initiatedByUserId.asc().nullsLast().op("int8_ops")),
	index("match_run_initiated_by_user_id_84c6e507").using("btree", table.initiatedByUserId.asc().nullsLast().op("int8_ops")),
	index("match_run_run_typ_e4c0d2_idx").using("btree", table.runType.asc().nullsLast().op("text_ops")),
	index("match_run_track_i_060ec1_idx").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("match_run_track_id_dd2b0774").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.initiatedByUserId],
			foreignColumns: [users.id],
			name: "match_run_initiated_by_user_id_84c6e507_fk_users_id"
		}),
	foreignKey({
			columns: [table.trackId],
			foreignColumns: [tracks.id],
			name: "match_run_track_id_dd2b0774_fk_tracks_id"
		}),
]);

export const matchRecommendation = pgTable("match_recommendation", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "match_recommendation_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("match_recom_accepte_50fa36_idx").using("btree", table.accepted.asc().nullsLast().op("bool_ops")),
	index("match_recom_group_i_56802c_idx").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	index("match_recom_match_r_e5666c_idx").using("btree", table.matchRunId.asc().nullsLast().op("int8_ops")),
	index("match_recom_mentor__d96f14_idx").using("btree", table.mentorUserId.asc().nullsLast().op("int8_ops")),
	index("match_recommendation_group_id_07fa28f8").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	index("match_recommendation_match_run_id_7af23874").using("btree", table.matchRunId.asc().nullsLast().op("int8_ops")),
	index("match_recommendation_mentor_user_id_c7242756").using("btree", table.mentorUserId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.groupId],
			foreignColumns: [groups.id],
	...
	unique("unique_match_recommendation_per_run_group_mentor").on(table.groupId, table.mentorUserId, table.matchRunId),
]);
...
export const roleAssignmentHistory = pgTable("role_assignment_history", {
	...
	index("role_assign_role_id_e11f96_idx").using("btree", table.roleId.asc().nullsLast().op("int8_ops")),
	index("role_assign_user_id_4edae0_idx").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	index("role_assignment_history_role_id_807250dc").using("btree", table.roleId.asc().nullsLast().op("int8_ops")),
	index("role_assignment_history_user_id_1b78a336").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.roleId],
			foreignColumns: [roles.id],
...
export const resourceAudience = pgTable("resource_audience", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "resource_audience_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	trackId: bigint("track_id", { mode: "number" }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	resourceId: bigint("resource_id", { mode: "number" }).notNull(),
	...
	index("resource_au_resourc_cef2c2_idx").using("btree", table.resourceId.asc().nullsLast().op("int8_ops")),
	index("resource_au_role_id_9287b0_idx").using("btree", table.roleId.asc().nullsLast().op("int8_ops")),
	index("resource_au_track_i_8569c2_idx").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("resource_audience_resource_id_4a807083").using("btree", table.resourceId.asc().nullsLast().op("int8_ops")),
	index("resource_audience_role_id_34580b4e").using("btree", table.roleId.asc().nullsLast().op("int8_ops")),
	index("resource_audience_track_id_8c2cfbea").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	uniqueIndex("unique_resource_role_audience").using("btree", table.resourceId.asc().nullsLast().op("int8_ops"), table.roleId.asc().nullsLast().op("int8_ops")).where(sql`((track_id IS NULL) AND (role_id IS NOT NULL))`),
	uniqueIndex("unique_resource_role_track_audience").using("btree", table.resourceId.asc().nullsLast().op("int8_ops"), table.roleId.asc().nullsLast().op("int8_ops"), table.trackId.asc().nullsLast().op("int8_ops")).where(sql`((role_id IS NOT NULL) AND (track_id IS NOT NULL))`),
	uniqueIndex("unique_resource_track_audience").using("btree", table.resourceId.asc().nullsLast().op("int8_ops"), table.trackId.asc().nullsLast().op("int8_ops")).where(sql`((role_id IS NULL) AND (track_id IS NOT NULL))`),
	foreignKey({
			columns: [table.resourceId],
			foreignColumns: [resources.id],
	...
	check("resource_audience_requires_role_or_track", sql`(role_id IS NOT NULL) OR (track_id IS NOT NULL)`),
]);
...
export const loginTokens = pgTable("login_tokens", {
	...
	index("login_token_user_id_e66ea4_idx").using("btree", table.userId.asc().nullsLast().op("int8_ops"), table.token.asc().nullsLast().op("int8_ops")),
	index("login_tokens_user_id_7f12777f").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
...
export const milestone = pgTable("milestone", {
	...
	index("milestone_complet_c0474d_idx").using("btree", table.completed.asc().nullsLast().op("bool_ops")),
	index("milestone_deleted_737b8a_idx").using("btree", table.deletedAt.asc().nullsLast().op("timestamptz_ops")),
	index("milestone_group_i_aa2495_idx").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	index("milestone_group_id_11a6a79d").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.groupId],
			foreignColumns: [groups.id],
...
export const tasks = pgTable("tasks", {
	...
	index("tasks_deleted_1d9a30_idx").using("btree", table.deletedAt.asc().nullsLast().op("timestamptz_ops")),
	index("tasks_due_dat_0359a9_idx").using("btree", table.dueDate.asc().nullsLast().op("timestamptz_ops")),
	index("tasks_milesto_5735eb_idx").using("btree", table.milestoneId.asc().nullsLast().op("int8_ops")),
	index("tasks_milestone_id_bdae7b7f").using("btree", table.milestoneId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const taskAssignees = pgTable("task_assignees", {
	...
	index("ta_active_by_task").using("btree", table.taskId.asc().nullsLast().op("int8_ops")).where(sql`(deleted_at IS NULL)`),
	index("ta_active_by_user").using("btree", table.userId.asc().nullsLast().op("int8_ops")).where(sql`(deleted_at IS NULL)`),
	index("task_assign_assigne_94a93f_idx").using("btree", table.assignedDatetime.asc().nullsLast().op("timestamptz_ops")),
	index("task_assign_task_id_39473c_idx").using("btree", table.taskId.asc().nullsLast().op("int8_ops")),
	index("task_assign_user_id_504562_idx").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	index("task_assignees_task_id_dfd06a60").using("btree", table.taskId.asc().nullsLast().op("int8_ops")),
	index("task_assignees_user_id_6b1c6501").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	foreignKey({
...
export const userSession = pgTable("user_session", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "user_session_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	index("user_sessio_expires_85bda6_idx").using("btree", table.expiresAt.asc().nullsLast().op("timestamptz_ops")),
	index("user_sessio_last_ac_a7b43b_idx").using("btree", table.lastActivityAt.asc().nullsLast().op("timestamptz_ops")),
	index("user_sessio_sid_1a73c0_idx").using("btree", table.sid.asc().nullsLast().op("text_ops")),
	index("user_sessio_user_id_c6fc39_idx").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	index("user_session_sid_17ee7496_like").using("btree", table.sid.asc().nullsLast().op("varchar_pattern_ops")),
	index("user_session_user_id_babcef0a").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
	...
	check("user_session_activity_after_created", sql`last_activity_at >= created_at`),
	check("user_session_ended_after_created", sql`(ended_at >= created_at) OR (ended_at IS NULL)`),
	check("user_session_expires_after_created", sql`expires_at >= created_at`),
	check("user_session_revoked_after_created", sql`(revoked_at >= created_at) OR (revoked_at IS NULL)`),
]);

export const alert = pgTable("alert", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "alert_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).notNull(),
	...
	index("alert_created_417cb1_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamptz_ops")),
	index("alert_resolve_79070e_idx").using("btree", table.resolved.asc().nullsLast().op("bool_ops")),
	index("alert_session_0863a0_idx").using("btree", table.sessionId.asc().nullsLast().op("int8_ops")),
	index("alert_session_id_5bdcb2f5").using("btree", table.sessionId.asc().nullsLast().op("int8_ops")),
	foreignKey({
	...
	check("alert_resolved_after_created", sql`(resolved_at >= created_at) OR (resolved_at IS NULL)`),
	check("alert_resolved_matches_timestamp", sql`((NOT resolved) AND (resolved_at IS NULL)) OR (resolved AND (resolved_at IS NOT NULL))`),
]);
...
export const workshops = pgTable("workshops", {
	...
	index("workshops_group_i_f53249_idx").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	index("workshops_group_id_452b7f33").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	index("workshops_host_us_e187e4_idx").using("btree", table.hostUserId.asc().nullsLast().op("int8_ops")),
	index("workshops_host_user_id_c0c274b0").using("btree", table.hostUserId.asc().nullsLast().op("int8_ops")),
	index("workshops_start_d_ee3d73_idx").using("btree", table.startDatetime.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
			columns: [table.groupId],
			foreignColumns: [groups.id],
...
export const workshopAttendance = pgTable("workshop_attendance", {
	...
	index("workshop_attendance_user_id_c7587c90").using("btree", table.userId.asc().nullsLast().op("int8_ops")),
	index("workshop_attendance_workshop_id_15b47095").using("btree", table.workshopId.asc().nullsLast().op("int8_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
...
export const resources = pgTable("resources", {
	...
	index("resources_deleted_77ce14_idx").using("btree", table.deletedAt.asc().nullsLast().op("timestamptz_ops")),
	index("resources_group_id_5680e4d8").using("btree", table.groupId.asc().nullsLast().op("int8_ops")),
	index("resources_resource_type_id_8ca31a8a").using("btree", table.typeId.asc().nullsLast().op("int8_ops")),
	index("resources_track_i_c53699_idx").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("resources_track_id_c4079c5a").using("btree", table.trackId.asc().nullsLast().op("int8_ops")),
	index("resources_uploade_d26da4_idx").using("btree", table.uploadedById.asc().nullsLast().op("int8_ops")),
	index("resources_uploader_user_id_id_f9418bc1").using("btree", table.uploadedById.asc().nullsLast().op("int8_ops")),
	index("resources_visibil_2b2301_idx").using("btree", table.visibilityScope.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.groupId],
			foreignColumns: [groups.id],
