...
export function RichEditor({ value, onChange, placeholder }: Props) {
    ...
    try {
      for (const [index, file] of files.entries()) {
        ...
        insertAttachmentLink(
          resource.accessUrl ?? resource.url,
      ...
      console.error(error);
      toast.error(
        "Attachment upload failed. Please check the file type and try again.",
      );
    } finally {
  ...
  const Sep = () => (
    <div className="w-px h-4 bg-border mx-0.5 self-center shrink-0" />
  );
