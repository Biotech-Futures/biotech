...
# Canonical app-root patterns. ``config.urls`` mounts this public list at
# ``/api/v1/events/...`` while this module keeps the legacy ``/events/v1/...``
# alias below. Specific paths come BEFORE ``*router.urls`` so the router's
# catch-all ``<pk>/`` detail pattern doesn't shadow them.
canonical_urlpatterns = [
...
urlpatterns = [
    ...
    path("v1/", include(canonical_urlpatterns)),
    *canonical_urlpatterns,
]
