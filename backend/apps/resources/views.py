...
class RoleAssignmentHistoryViewSet(mixins.UpdateModelMixin,
        ...
        try:
            ...
            if not force:
                ...
                if active_roles.exists() and not revoke_others:
                    active_role_names = [ar.role.slug for ar in active_roles]
                    return Response(
                        {
            ...
            response_data = {
                "message": result.get('message', f"Role '{role.slug}' granted to user '{user.email}'"),
    ...
    def revoke_role(self, request):
        ...
        try:
            ...
            return Response(
                {
                    "message": f"Role '{role.slug}' revoked from user '{user.email}'",
                    "details": {
                        "user_id": user.id,
                        "role_id": role.id,
...
class ResourcesViewSet(mixins.ListModelMixin,
    ...
    def _apply_filters(self, queryset):
        ...
        if role:
            queryset = queryset.filter(audiences__role__slug__icontains=role)
    ...
    def _user_can_access_resource(self, user, resource):
        ...
        resource_role_names = [aud.role.slug for aud in audiences if aud.role_id]
        resource_track_names = [aud.track.track_code for aud in audiences if aud.track_id]
    ...
    def assign_role(self, request, pk=None):
        ...
        try:
            ...
            return Response({
                "message": f"Role '{role.slug}' assigned to resource '{resource.resource_name}'",
