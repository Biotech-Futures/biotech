...
# ---------------------------------------------------------------------------
# Unit-level view tests (mock service, no DB fixtures)
# ---------------------------------------------------------------------------

def _make_group_mock(**overrides):
...
class GroupsPreviewViewTest(TestCase):
    """
    Unit tests for GroupsPreviewView that mock the service layer.

    Verifies serializer output shape, query-param forwarding, pagination,
    and validation without requiring real DB fixtures.
    """
    ...
    def test_returns_paginated_response(self, mock_service):
        ...
        mock_service.return_value = [_make_group_mock(id=12)]
    ...
    def test_lead_user_null_when_no_mentor(self, mock_service):
        ...
        mock_service.return_value = [
            _make_group_mock(lead_user_id=None, lead_first_name=None, lead_last_name=None)
        ]
    ...
    def test_pagination_page_size(self, mock_service):
        ...
        mock_service.return_value = [
            _make_group_mock(id=i, group_name=f"Group{i}", lead_user_id=None,
