...
class InactiveLoginGateTests(TestCase):
    ...
    # --- magic link ------------------------------------------------------
    #
    # The GET is a side-effect-free handoff (mail scanners prefetch it), so the
    # status gate now lives on the POST the "Continue" button makes. These assert
    # the gate still holds on the path that actually opens the session.

    def test_magic_link_confirm_blocks_deactivated_account(self):
        code = self._fresh_code()
        self.user.deactivate()
        ...
        self.assertEqual(handoff.status_code, status.HTTP_302_FOUND)
        self.assertIn(f"code={code}", handoff["Location"])
    ...
    def test_magic_link_confirm_blocks_suspended_account(self):
        code = self._fresh_code()
        self.user.suspend()

        self.client.get(reverse("magic_login"), {"email": self.email, "code": code})
        response = self.client.post(
            reverse("verify_login_code"),
            {"email": self.email, "code": code},
            format="json",
    ...
    def test_magic_link_confirm_allowed_for_invited_and_pending(self):
        for transition in ("invite", "mark_pending"):
            with self.subTest(transition=transition):
                cache.clear()
                code = self._fresh_code()
