// Ported from adminweb/src/lib/crop-event-image.ts so the Vue admin produces
// the same fixed 1280x320 banner the backend now hard-requires
// (backend/apps/admin/services/event_image.py rejects anything else).
...
export function getMaximumEventCropBounds(imageWidth: number, imageHeight: number): PixelRect {
...
export async function cropEventImage(sourceUrl: string, crop: EventCropRect): Promise<File> {
  const image = await loadImage(sourceUrl)
  const maximum = getMaximumEventCropBounds(image.naturalWidth, image.naturalHeight)
