<template>
  ...
  <!--
    中文：
    顶层分栏登录页面容器。该区域负责把页面拆分为“左侧品牌展示区”和“右侧登录操作区”两个并列模块。
    关联变量与样式包括：
    1. split-login：控制整体两栏网格布局、最小高度和响应式折叠逻辑。
    2. 左侧 section：承载品牌叙事、背景特效、国际化文本与外链按钮。
    3. 右侧 section：承载邮箱输入、OTP 验证、状态反馈与语言切换。
    运作原理：
    这是纯前端布局骨架，本身不直接请求后端；它的作用是为后续的登录交互、Three.js 背景渲染和多语言渲染提供稳定的 DOM 结构。
    English:
    Root split-layout container for the whole login page. It separates the view into a left branding hero area and a right authentication workspace.
    Related pieces:
    1. split-login controls the overall grid, viewport height, and responsive collapse behavior.
    2. The left section hosts branding copy, animated backgrounds, localized text, and the website link.
    3. The right section hosts email submission, OTP verification, feedback states, and language switching.
    Runtime note:
    This wrapper itself is a front-end layout shell. It does not talk to the backend directly, but it gives the auth flow, Three.js scene, and i18n rendering a stable DOM structure to work with.
    -->
  <div class="split-login">
    ...
    <!--
      中文：
      左侧品牌展示区。这里是页面的视觉主舞台，负责承载背景轮播、Three.js 生物科技场景、品牌标题、角色标签和介绍文案。
      关联变量：
      1. leftPaneRef：供脚本层绑定 pointermove、pointerleave 和 ResizeObserver，用于 Three.js 交互和尺寸同步。
      2. activeLeftBackground：决定当前启用的是图片轮播背景还是 biotech 3D 背景。
      3. currentDir：根据语言方向动态切换 LTR/RTL 布局。
      前后端关系：
      该区域本身不直接与后端通信，属于纯前端展示层；但它通过共享的 locale、t()、leftHtml 等状态，和右侧登录区一起保持一致的国际化体验。
      English:
      Left branding hero section. This is the main visual stage of the page and contains the image slideshow, the Three.js biotech scene, the brand heading, role chips, and the marketing copy.
      Related state:
      1. leftPaneRef is used by the script layer for pointer events and ResizeObserver so the Three.js scene can react to mouse movement and container resizing.
      2. activeLeftBackground decides whether the slideshow or the biotech 3D mode is visible.
      3. currentDir switches layout direction for LTR and RTL languages.
      Front-end / back-end note:
      This section is presentation-only and does not call the backend directly. It shares locale-driven state with the login panel so both sides stay synchronized visually.
      -->
    <section ref="leftPaneRef" class="left-pane" :class="`bg-mode-${activeLeftBackground}`">
      ...
      <!--
        中文：
        左侧背景堆叠容器。这个容器专门负责管理“多层背景”的层级关系，让静态/轮播图片背景与 Three.js 动态背景可以共存，并通过 active class 平滑切换。
        关联变量：
        1. backgroundImages：轮播图片数组。
        2. slideDuration：每张图的动画延迟基准。
        3. activeLeftBackground：决定哪一层背景处于 active 状态。
        原理：
        这里采用绝对定位叠层设计，背景层不参与普通文档流，上方文字区单独保持在更高 z-index，避免背景切换影响文案布局。
        English:
        Background stacking container for the left pane. It manages multiple visual layers so the slideshow and the Three.js scene can coexist and switch smoothly via active classes.
        Related state:
        1. backgroundImages supplies the slideshow assets.
        2. slideDuration defines the stagger timing between slides.
        3. activeLeftBackground decides which background layer becomes active.
        How it works:
        The backgrounds are absolutely positioned layers, while the text content remains on a higher z-index, so visual switching never disturbs the content layout.
        -->
      <div class="left-bg-stack">
        ...
        <div
          ...
          aria-hidden="true"
        >
          <!--但不是简单的静态背景，而是js动态渲染特效的载点，在页面加载后把特效渲染进这个DOM里-->
      ...
      <!--
      中文：
      背景模式切换按钮。点击后会调用 toggleLeftBackground()，切换 activeLeftBackground 的值，从而在图片轮播与 3D 生物科技场景之间切换。
      关联计算属性：
      1. backgroundToggleText：控制按钮上显示 DNA / IMG 等简短标签。
      2. backgroundToggleLabel：提供无障碍 aria-label 与 title 提示。
      3. toggleLeftBackground()：真正修改响应式状态的动作函数。
      English:
      Background mode switch button. It calls toggleLeftBackground() to swap activeLeftBackground between the slideshow mode and the biotech 3D mode.
      Related computed values:
      1. backgroundToggleText controls the compact button label such as DNA or IMG.
      2. backgroundToggleLabel provides accessible aria-label and hover title text.
      3. toggleLeftBackground() performs the actual state mutation.
      -->
      <button
        type="button"
      ...
      <!--
      中文：
      左侧文字玻璃卡片。这里集中承载品牌 Logo、系统标题、角色标签、介绍文案和官网跳转按钮。
      关联变量：
      1. currentDir：决定文本方向，阿拉伯语场景会切换成 rtl。
      2. t(key)：所有静态文案都通过国际化函数读取。
      3. leftHtml：用计算属性拼装介绍区的 HTML 内容，并通过 v-html 注入。
      维护提醒：
      因为 leftHtml 使用了 v-html，后续若改为从接口获取富文本，必须做好内容可信度控制与 XSS 审核。
      English:
      Frosted content panel on the left. It groups the logo, product title, role chips, introduction copy, and the external website link.
      Related pieces:
      1. currentDir controls text direction and switches to rtl for Arabic.
      2. t(key) is used to resolve localized strings.
      3. leftHtml builds the informational content as computed HTML and injects it with v-html.
      Maintenance note:
      Because leftHtml is rendered with v-html, any future backend-driven rich text must be sanitized carefully to avoid XSS risks.
      -->
    ...
    <!--
      中文：
      右侧登录操作区。该区域是真正处理用户认证交互的业务入口，负责邮箱提交、验证码输入、接口调用结果反馈以及语言切换。
      关联变量：
      1. currentStep：控制当前显示邮箱输入页还是 OTP 验证页。
      2. email、otpDigits：保存用户输入。
      3. sendingCode、verifyingCode、resendingCode：控制异步按钮状态和加载动画。
      4. error、statusMessage：显示错误或成功提示。
      前后端协作：
      右侧区域通过 handleLogin() 和 verifyOTP() 等函数调用 Django 后端接口，后端负责生成/校验一次性验证码，前端负责渲染状态与跳转逻辑。
      English:
      Right-side authentication workspace. This is the actual business entry for login interactions, including email submission, OTP input, async feedback, and language switching.
      Related state:
      1. currentStep determines whether the email view or OTP view is shown.
      2. email and otpDigits store user input.
      3. sendingCode, verifyingCode, and resendingCode drive disabled/loading states.
      4. error and statusMessage display feedback to the user.
      Front-end / back-end collaboration:
      This section calls the Django endpoints through handleLogin() and verifyOTP(). The backend generates and validates one-time codes, while the frontend renders state and performs navigation.
      -->
    <section class="right-pane">
      <div class="login-card fade-in" :dir="currentDir" :class="{ rtl: currentDir === 'rtl' }">
        ...
        <!--
          中文：
          邮箱提交步骤。这个步骤是密码less 登录流程的第一阶段，用户输入邮箱后，前端会先做格式校验，再把 email 和 redirect_url 发送给后端。
          关联变量与函数：
          1. email：通过 v-model.trim 双向绑定输入框内容。
          2. handleLogin()：提交表单的主函数。
          3. sendingCode / resendCountdown：控制按钮禁用与倒计时文案。
          4. error / statusMessage：承载校验或发送结果提示。
          流程说明：
          前端不直接生成验证码，而是把邮箱交给后端的 send-login-code 接口，由后端决定发送策略、验证码有效期和安全限制。
          English:
          Email submission step. This is the first phase of the passwordless login flow. The frontend validates the email first, then sends email and redirect_url to the backend.
          Related state and functions:
          1. email is two-way bound with v-model.trim.
          2. handleLogin() is the main submission function.
          3. sendingCode and resendCountdown control disabled states and countdown text.
          4. error and statusMessage hold validation and request feedback.
          Flow note:
          The frontend does not generate the OTP itself. It hands the email to the backend send-login-code endpoint, where expiry rules and security policies are enforced.
          -->
        ...
        <!--
          中文：
          OTP 验证步骤。该步骤在验证码发送成功后展示，用户在 6 个输入框中录入一次性验证码，前端负责输入体验优化，后端负责真实性校验。
          关联变量与函数：
          1. otpDigits：保存 6 位验证码数组。
          2. otpRefs：保存每个输入框 DOM 引用，便于自动跳焦点。
          3. handleOTPInput / handleOTPKeydown / handleOTPPaste：优化输入、删除、粘贴等交互。
          4. verifyOTP()：向后端发送 email + code 进行校验。
          5. maskedEmail：对已发送目标邮箱做脱敏展示。
          English:
          OTP verification step. It appears after the code is sent. The frontend optimizes the six-slot input experience, while the backend verifies whether the one-time code is valid.
          Related state and functions:
          1. otpDigits stores the six code characters.
          2. otpRefs stores DOM references for focus automation.
          3. handleOTPInput, handleOTPKeydown, and handleOTPPaste improve typing, deleting, and paste behavior.
          4. verifyOTP() submits email plus code to the backend.
          5. maskedEmail displays a privacy-safe version of the target email.
          -->
        ...
        <!--
          中文：
          语言切换区域。这里通过 languageOptions 批量生成按钮，并调用 switchLanguage() 同步切换 locale、清空反馈状态并持久化到 localStorage。
          关联变量：
          1. languageOptions：可选语言数组。
          2. locale：当前语言值。
          3. switchLanguage()：切换语言并保存偏好。
          4. currentDir：当 locale 为 ar 时自动切换 rtl 布局。
          English:
          Language switcher area. Buttons are rendered from languageOptions and call switchLanguage() to update locale, clear transient messages, and persist the preference to localStorage.
          Related pieces:
          1. languageOptions defines all supported languages.
          2. locale holds the active language.
          3. switchLanguage() updates and persists the preference.
          4. currentDir automatically turns the layout into rtl when locale is ar.
          -->
...
/*
  中文：
  该脚本是登录页的业务核心，主要分为五个模块：
  1. 基础依赖与资源导入：引入 Vue、Three.js、路由、认证 store 和静态资源。
  2. 页面状态管理：维护邮箱、验证码、错误提示、背景模式、语言和 DOM 引用。
  3. 登录认证逻辑：负责发送验证码、校验验证码、跳转到登录后的页面。
  4. 输入体验增强：负责 OTP 自动跳格、删除逻辑、粘贴逻辑和倒计时。
  5. Three.js 背景系统：负责初始化、渲染、交互、尺寸同步和销毁左侧生物科技场景。
  前后端协作方式：
  前端通过 fetch 调用 Django 后端接口 /services/send-login-code/ 与 /services/verify-login-code/。
  后端负责验证码生成、单次使用限制、有效期控制和会话建立；前端负责界面状态、用户交互和登录后的跳转。

  English:
  This script is the business core of the login page and is organized into five modules:
  1. Dependency and asset imports for Vue, Three.js, routing, auth store, and static assets.
  2. Page state management for email, OTP, feedback, background mode, locale, and DOM refs.
  3. Authentication logic for sending the login code, verifying the OTP, and redirecting after login.
  4. Input experience helpers for auto-advance, deletion, paste handling, and resend countdown.
  5. Three.js background orchestration for scene setup, rendering, interaction, resize sync, and cleanup.
  Front-end / back-end collaboration:
  The frontend calls the Django endpoints /services/send-login-code/ and /services/verify-login-code/ through fetch.
  The backend owns OTP generation, single-use enforcement, expiry control, and session creation, while the frontend owns view state, interaction flow, and post-login navigation.
*/
...
/*
  中文：
  基础依赖实例与静态配置。
  1. router：登录成功后负责前端路由跳转。
  2. auth：Pinia 认证 store，用来刷新当前用户信息。
  3. backgroundImages 与 slideDuration：控制左侧轮播背景。
  4. API_BASE_URL：统一管理后端 API 前缀，便于开发环境和部署环境切换。
  English:
  Core runtime instances and static configuration.
  1. router handles client-side navigation after a successful login.
  2. auth is the Pinia auth store used to refresh the current user.
  3. backgroundImages and slideDuration drive the slideshow background.
  4. API_BASE_URL centralizes the backend API prefix for easier environment switching.
*/
...
/*
  中文：
  响应式页面状态区。
  这些 ref 共同描述当前登录页的“输入状态、网络状态、界面状态和 DOM 引用状态”。
  维护建议：
  1. 与邮箱登录流程直接相关的状态尽量集中放在一起，避免后期扩展时分散在多个区域。
  2. 布尔值命名统一使用进行中语义，例如 sendingCode、verifyingCode，阅读时更清晰。
  English:
  Reactive page state section.
  These refs describe the current input state, network state, UI state, and DOM reference state of the login page.
  Maintenance tips:
  1. Keep email-login-related state together so future auth changes do not become scattered.
  2. Use progressive boolean names such as sendingCode and verifyingCode for clearer intent.
*/
const email = ref('')
...
/*
  中文：
  左侧背景模式切换逻辑。
  该模块只关心视觉状态，不依赖后端。activeLeftBackground 改变后，模板中的动态 class 会触发不同背景层的显示与隐藏。
  English:
  Left hero background switching logic.
  This module is purely visual and does not depend on the backend. When activeLeftBackground changes, dynamic classes in the template toggle which background layer is visible.
*/
...
/*
  中文：
  国际化配置区。
  languageOptions 定义语言切换按钮，messages 维护多语言文案映射。
  维护建议：
  1. 所有新增文案 key 应在所有语言中保持一致，避免回退英文时出现缺项。
  2. 文案量继续增大时，建议拆分到独立 i18n 文件，降低单组件体积。
  English:
  Internationalization configuration section.
  languageOptions defines the switcher buttons, while messages stores localized copy.
  Maintenance tips:
  1. Keep every language object aligned on the same keys to avoid accidental fallback gaps.
  2. If the copy grows further, consider extracting it into dedicated i18n modules to reduce component size.
*/
...
/*
  ...
  国际化读取与派生 UI 状态。
  1. t(key) 是统一的文案读取入口。
  2. currentDir 负责在阿拉伯语场景下切换 rtl。
  3. leftHtml、isOtpComplete、maskedEmail 都是直接服务模板渲染的计算属性。
  English:
  i18n accessors and derived UI state.
  1. t(key) is the unified localization accessor.
  2. currentDir flips the layout into rtl for Arabic.
  3. leftHtml, isOtpComplete, and maskedEmail are computed values consumed directly by the template.
*/
...
/*
  中文：
  OTP 输入体验增强区。
  这里的函数只负责“更顺手地输入验证码”，不直接处理后端认证结果。
  主要职责：
  1. 保存 6 个输入框 DOM 引用。
  2. 自动跳到下一格。
  3. 支持退格回删上一格。
  4. 支持一次性粘贴 6 位验证码。
  5. 在按 Enter 时触发 verifyOTP()。
  English:
  OTP input experience enhancement section.
  These functions only improve how users type the code and do not own backend verification.
  Responsibilities:
  1. Store DOM refs for the six inputs.
  2. Auto-advance to the next slot.
  3. Support backward delete behavior.
  4. Support pasting a full six-digit code.
  5. Trigger verifyOTP() when Enter is pressed.
*/
// 保存验证码每一位值
...
/*
  中文：
  通用工具函数区。
  这里放的是多个业务步骤都会依赖的小工具，例如邮箱校验、错误消息解析、回调地址生成和重发倒计时。
  English:
  Shared utility helpers.
  This section contains small helpers reused across multiple business steps, such as email validation, error parsing, callback URL generation, and resend countdown handling.
*/
// 判断是否是合法邮箱格式
...
/*
  ...
  这部分函数会真正与后端接口交互：
  1. handleLogin()：发送验证码。
  2. verifyOTP()：校验验证码并拉取用户信息。
  3. resendCode()：重新发送验证码。
  4. resolvePostLoginRoute()：根据用户角色决定登录后的落点。
  关键协作关系：
  前端负责校验输入、展示 loading 和提示信息；后端负责发码、验码、会话建立和用户态返回。
  English:
  Main authentication workflow section.
  These functions perform the actual API interaction:
  1. handleLogin() sends the verification code.
  2. verifyOTP() validates the OTP and refreshes user data.
  3. resendCode() sends a replacement code.
  4. resolvePostLoginRoute() decides the destination route after login.
  Collaboration model:
  The frontend validates input and renders loading and feedback, while the backend owns code delivery, verification, session creation, and authenticated user state.
*/
// 邮件提交发送验证码主函数
...
/*
  中文：
  Three.js 生物科技背景模块。
  该模块是一个完全独立于登录业务的视觉子系统，用于在左侧展示具有品牌感的动态 biotech 场景。
  模块组成：
  1. Three.js 运行时变量：scene、camera、renderer、clock 等。
  2. 材质与几何构建函数：createSoftMaterial()、createDnaHelix() 等。
  3. 场景初始化与动画循环：setupBiotechScene()、animateBiotechScene()。
  4. 交互与生命周期：鼠标移动、离开、resize、mounted、beforeUnmount。
  维护提醒：
  若后续要继续添加新的生物模型，优先沿用 addFloatingItem() 的注册方式，保持动画和交互统一。
  English:
  Three.js biotech background module.
  This is a visual subsystem fully decoupled from the auth workflow and is responsible for the animated biotech hero scene on the left.
  Module parts:
  1. Runtime variables such as scene, camera, renderer, and clock.
  2. Material and geometry builders such as createSoftMaterial() and createDnaHelix().
  3. Scene setup and animation loop via setupBiotechScene() and animateBiotechScene().
  4. Interaction and lifecycle hooks for mouse movement, leave, resize, mount, and cleanup.
  Maintenance note:
  If new biotech models are added later, prefer registering them through addFloatingItem() so animation and interaction behavior stay consistent.
*/
...
/*
  中文：
  Three.js 运行时对象与交互缓存。
  这些变量不使用 ref，是因为它们不需要驱动 Vue 模板重渲染，而是只在渲染循环内部维护 WebGL 状态。
  English:
  Three.js runtime objects and interaction caches.
  These values are not refs because they do not need to trigger Vue re-rendering. They only maintain WebGL state inside the rendering loop.
*/
let scene = null
let camera = null
let renderer = null
...
/*
  中文：
  材质与场景对象注册辅助函数。
  1. createSoftMaterial()：统一生成具有玻璃/软质质感的材质。
  2. addFloatingItem()：把模型注册进场景，同时保存其基础位置、速度和受力参数，供动画循环统一更新。
  English:
  Material and scene registration helpers.
  1. createSoftMaterial() creates a consistent glossy or soft translucent material.
  2. addFloatingItem() registers a model into the scene and stores its base position, velocity, and force factor for the animation loop.
*/
...
/*
  中文：
  生物科技模型构建函数区。
  每个 createXxxModel/createXxx 函数负责生成一个 Three.js Group 或 Mesh，并返回给 setupBiotechScene() 统一挂载。
  当前模型包括：
  1. 细菌
  2. 蛋白复合体
  3. 胶囊药片
  ...
  6. DNA 双螺旋
  English:
  Biotech model factory section.
  Each createXxxModel/createXxx function builds a Three.js Group or Mesh and returns it so setupBiotechScene() can mount it consistently.
...
/*
  中文：
  粒子与氛围动画模块。
  createBreathingParticles() 负责初始化背景粒子，animateBreathingParticles() 在每一帧中更新粒子的位置、透明度和整体旋转，让场景更有生命感。
  English:
  Particle and ambience animation module.
  createBreathingParticles() initializes the background particles, and animateBreathingParticles() updates their motion, opacity, and rotation every frame to make the scene feel alive.
*/
...
/*
  中文：
  场景初始化与渲染循环。
  1. setupBiotechScene()：创建 scene、camera、renderer、灯光与模型。
  2. updateMouseWorld()：把鼠标位置从屏幕坐标转换为世界坐标，用于模型躲避鼠标。
  3. animateBiotechScene()：逐帧更新浮动物体、粒子和镜头，并执行 renderer.render()。
  English:
  Scene initialization and rendering loop.
  1. setupBiotechScene() creates the scene, camera, renderer, lights, and models.
  2. updateMouseWorld() maps pointer coordinates into world space so objects can react to the cursor.
  3. animateBiotechScene() updates floating objects, particles, and the camera every frame, then calls renderer.render().
*/
...
/*
  中文：
  生命周期管理区。
  onMounted() 负责初始化 3D 场景并绑定 resize / pointer 事件。
  onBeforeUnmount() 负责清理定时器、动画帧、监听器和 WebGL 资源，避免页面离开后发生内存泄漏。
  English:
  Lifecycle management section.
  onMounted() initializes the 3D scene and binds resize and pointer listeners.
  onBeforeUnmount() clears timers, animation frames, event listeners, and WebGL resources to avoid memory leaks after the page is destroyed.
*/
onMounted(() => {
  setupBiotechScene()
  handleSceneResize()
...
/*
  中文：
  样式层按照“页面布局 → 左侧品牌区 → 右侧登录区 → 表单与按钮 → OTP 与反馈 → 动画 → 响应式”进行分组。
  这样做的目的，是让后续维护时可以先按模块定位，再进入具体选择器调优。
  English:
  The stylesheet is grouped by page layout, left branding area, right login area, form and buttons, OTP and feedback, animations, then responsive rules.
  This makes maintenance easier because you can find the right module first before fine-tuning individual selectors.
*/

/*
  中文：
  页面整体布局。
  split-login、left-pane、right-pane 决定双栏结构、视口占满方式以及左右区块的基础背景。
  English:
  Page-level layout.
  split-login, left-pane, and right-pane define the two-column structure, viewport coverage, and the base backgrounds of both sides.
*/
.split-login {
  display: grid;
...
/*
  中文：
  左侧背景层系统。
  这里的样式控制图片轮播层、Three.js 画布层、遮罩层以及背景切换按钮。
  English:
  Left-side background layering system.
  These rules control the slideshow layer, Three.js canvas layer, overlay layer, and the background toggle button.
*/
.left-bg-stack {
  position: absolute;
  inset: 0;
...
/*
  中文：
  左侧玻璃内容卡片与品牌文案。
  包括 left-inner、brand、logo-icon、custom-content、role-tags 和外链按钮等元素。
  English:
  Left frosted content card and brand copy.
  This group covers left-inner, brand, logo-icon, custom-content, role-tags, and the external CTA button.
*/
.left-inner {
  position: relative;
  z-index: 1;
...
/*
  中文：
  右侧登录卡片骨架。
  这一组样式控制 right-pane、login-card、logo、标题、副标题和滚动表现。
  English:
  Right-side login card shell.
  This group styles right-pane, login-card, the logo area, headings, subtitles, and card scrolling behavior.
*/
.right-pane {
...
/*
  中文：
  通用表单控件与主按钮。
  包括输入框、标签、说明文字、按钮基础类以及主次按钮视觉。
  English:
  Shared form controls and primary actions.
  This group includes inputs, labels, helper text, base button styles, and primary or secondary button visuals.
*/
.form-group {
  margin-bottom: 1rem;
}
...
/*
  中文：
  OTP 输入区、状态反馈和辅助交互。
  包括验证码输入格、提示消息、帮助链接、语言切换器、步骤切换动画等。
  English:
  OTP area, feedback states, and helper interactions.
  This group covers OTP boxes, feedback banners, help links, the language switcher, and step transition visuals.
*/
.otp-container {
  display: flex;
  gap: 0.6rem;
...
/*
  中文：
  动画定义。
  这里集中放置背景呼吸动画、轮播淡入淡出、步骤切换动效和 loading 旋转。
  English:
  Animation definitions.
  This section centralizes the overlay breathing motion, slideshow fade timing, step transitions, and loading spinner animation.
*/
...
/*
  中文：
  响应式适配。
  这里根据平板和手机尺寸收缩卡片、改单栏布局并压缩 OTP 和文案尺寸。
  English:
  Responsive adjustments.
  These rules shrink cards, collapse to a single column, and tighten OTP and text sizes for tablet and mobile widths.
*/
